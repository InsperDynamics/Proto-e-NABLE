...
SoftwareServo::SoftwareServo(uint8_t pinArg, int angleArg) : pin(pinArg), minAngle(40), maxAngle(170), min16(34), max16(150)
{
    next = first;
    first = this;
    pinMode(pin, OUTPUT);
...
void SoftwareServo::setMinimumAngle(int minAngleArg)
{
    if (minAngleArg < 0 || minAngleArg > 180 || minAngleArg > maxAngle)
        return;

    minAngle = minAngleArg;
}

void SoftwareServo::setMaximumAngle(int maxAngleArg)
{
    if (maxAngleArg < 0 || maxAngleArg > 180 || maxAngleArg < minAngle)
        return;

    maxAngle = maxAngleArg;
}
...
{
    if (angleArg < minAngle) {
        angleArg = minAngle;
    }
    if (angleArg > maxAngle) {
        angleArg = maxAngle;
    }
    angle = angleArg;
...
bool SoftwareServo::changeDirection() {
